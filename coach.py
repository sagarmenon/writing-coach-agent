...
— Your Coach"""
    ...
    # Get live reading rec
    reading_rec = get_live_reading_rec(dominant, patterns)
...
def receive_draft():
    ...
    if word_count > 80:
        ...
        metrics_sheet = format_metrics_for_sheet(metrics)
        
        # Log to Session Log sheet
        if sheets:
            reading_rec = result.get("reading_rec") or {}
            rec_str = f"{reading_rec.get('title', '')} by {reading_rec.get('author', '')}" if reading_rec else ""
            row = [
            ...
            # Log to Reading Queue
            if reading_rec and reading_rec.get("title"):
                append_sheet(sheets, "Reading Queue!A:E", [
                    datetime.now().strftime("%Y-%m-%d"),
                    reading_rec.get("title", ""),
                    reading_rec.get("author", ""),
                    reading_rec.get("why", ""),
                    "No"
                ])

        html = markdown_to_html(result["feedback"], metrics_text, result.get("reading_rec"))
